...
def match_template(np.ndarray[float, ndim=3, mode="c"] image,
                   np.ndarray[float, ndim=3, mode="c"] template):
    cdef np.ndarray[float, ndim=3, mode="c"] corr
    cdef np.ndarray[float, ndim=3, mode="c"] image_sat
    cdef np.ndarray[float, ndim=3, mode="c"] image_sqr_sat
    ...
    image_sqr_sat = np.cumsum(np.cumsum(np.cumsum(image**2,2),1),0)

    template -= template_mean
    template_ssd = np.sum(template**2)
    # use inversed area for accuracy
    inv_area = 1.0 / (template.shape[0] * template.shape[1] * template.shape[2])
    ...
    for i in range(corr.shape[0]):
        for j in range(corr.shape[1]):
            ...
            # summed-area table, instead of slicing windows of the image.
                ...
                k_end = k + template.shape[2] - 1
            
                window_sum = integrate(image_sat, i, j, k, i_end, j_end, k_end)
                window_mean_sqr = window_sum * window_sum * inv_area
                window_sqr_sum = integrate(image_sqr_sat, i, j, k, i_end, j_end, k_end)
