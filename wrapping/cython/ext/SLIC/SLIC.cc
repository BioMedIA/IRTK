	...
	const double&					zscale )
{
	...
	int xstrips = (0.5+double(m_width)/(STEP/xscale));
	int ystrips = (0.5+double(m_height)/(STEP/yscale));
	int zstrips = (0.5+double(m_depth)/(STEP/zscale));

    int xerr = m_width  - STEPX*xstrips;if(xerr < 0){xstrips--;xerr = m_width - STEPX*xstrips;}
    int yerr = m_height - STEPY*ystrips;if(yerr < 0){ystrips--;yerr = m_height- STEPY*ystrips;}
    int zerr = m_depth  - STEPZ*zstrips;if(zerr < 0){zstrips--;zerr = m_depth - STEPZ*zstrips;}
	...
	for( int z = 0; z < zstrips; z++ )
	{
		...
		for( int y = 0; y < ystrips; y++ )
		{
			...
			{
				int xe = x*xerrperstrip;
				int i = (y*STEPY+yoff+ye)*m_width + (x*STEPX+xoff+xe);
				
				kseedsl[n] = m_lvecvec[d][i];
				kseedsa[n] = m_avecvec[d][i];
				kseedsb[n] = m_bvecvec[d][i];
				kseedsx[n] = (x*STEPX+xoff+xe);
				kseedsy[n] = (y*STEPY+yoff+ye);
				kseedsz[n] = d;
				n++;
			}
		}
	...
	const double&				compactness)
{
	...
	//double invwt = 1.0/((STEP/compactness)*(STEP/compactness));//compactness = 20.0 is usually good.
        double invwtx = scalex*scalex/((STEP/compactness)*(STEP/compactness));
        double invwty = scaley*scaley/((STEP/compactness)*(STEP/compactness));
        double invwtz = scalez*scalez/((STEP/compactness)*(STEP/compactness));
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
                        y1 = max(0.0,			kseedsy[n]-offsety);
                        y2 = min((double)m_height,	kseedsy[n]+offsety);
                        x1 = max(0.0,			kseedsx[n]-offsetx);
                        x2 = min((double)m_width,	kseedsx[n]+offsetx);
			...
			for( int z = z1; z < z2; z++ )
			{
				for( int y = y1; y < y2; y++ )
				{
					for( int x = x1; x < x2; x++ )
					{
						...
						distxyz =		invwtx*(x - kseedsx[n])*(x - kseedsx[n]) +
										invwty*(y - kseedsy[n])*(y - kseedsy[n]) +
										invwtz*(z - kseedsz[n])*(z - kseedsz[n]);
						//------------------------------------------------------------------------
						//dist += distxyz*invwt;
                                                dist += distxyz;
						//------------------------------------------------------------------------
        ...
        const int&					STEPZ)
{
	...
	int sz = width*height;
	const int SUPSZ = STEPX*STEPY*STEPZ;
	...
	{for( int d = 0; d < depth; d++ )
	{
        ...
        //reenumerate (use contiguous labels starting from 1)
        // {
        //     int max = 0;
        //     for( int d = 0; d < depth; d++ )
	// 	for( int i = 0; i < sz; i++ ) {
        ...
        //     std::vector<int> flags(max,0);
        //     int label = 1;
        //     for( int d = 0; d < depth; d++ )
	// 	for( int i = 0; i < sz; i++ ) {
        //             if ( flags[nlabels[d][i]] == 0 ) {
        //                 flags[nlabels[d][i]] = label;
        //                 //label++;
        //             }
        //             labels[d][i] = flags[nlabels[d][i]];
        //         }
        //     std::cout << "max=" << max << "  last label = " << label << std::endl;
	...
	{for( int d = 0; d < depth; d++ )
	{
		for( int i = 0; i < sz; i++ ) labels[d][i] = nlabels[d][i]+1;
	}}
	{for( int d = 0; d < depth; d++ )
	{
		delete [] nlabels[d];
...
{
    //---------------------------------------------------------
    const double STEP = 0.5 + pow(double(supervoxelsize),1.0/3.0);
    const int STEPX = STEP / xscale;
    const int STEPY = STEP / yscale;
    const int STEPZ = STEP / zscale;
	...
	GetKValues_LABXYZ(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, kseedsz,
                          STEPX, STEPY, STEPZ,STEP,  xscale,yscale,zscale );

	PerformSupervoxelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy,
                              kseedsz, klabels, STEP, STEPX, STEPY, STEPZ, xscale,yscale,zscale, compactness);

	EnforceSupervoxelLabelConnectivity(klabels, width, height, depth, numlabels, STEPX, STEPY, STEPZ);
}
