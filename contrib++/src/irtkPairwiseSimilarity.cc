...
void irtkPairwiseSimilarity::Initialize(int nrSubjects, int nrSubjects2, bool useMasks, irtkGreyImage ** regions, int simType, int nrRegions){
...
void irtkPairwiseSimilarity::LoadImages(string imagefilename, string imageDir){
    ...
    from.close();
 	 ...
 	 if(files.size() != _nrSubjects && !(_twoSets && _nrRows + _nrCols == files.size())){
