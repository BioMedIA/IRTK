...
int main(int argc, char **argv)
{
  ...
  if ((x1 != 0) || (x2 != target.GetX()) ||
      (y1 != 0) || (y2 != target.GetY())) {
    target = target.GetRegion(x1, y1, 0, x2, y2, 1);
    source = source.GetRegion(x1, y1, 0, x2, y2, 1);
  }
  ...
  if (interpolator == NULL) {
    interpolator =
      new irtkNearestNeighborInterpolateImageFunction2D<irtkGreyPixel>;
  }
    ...
    for (y = 0; y < target.GetY(); y++) {
      for (x = 0; x < target.GetX(); x++) {
        if (target(x,y,z) > Tp) {
          ...
          if ((p._x > x1) && (p._x < x2) &&
              (p._y > y1) && (p._y < y2)) {
            histogram.AddSample(target(x, y, 0), round(interpolator->EvaluateInside(p._x, p._y, 0)));
          }
        }
      }
    }
