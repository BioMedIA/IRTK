...
void usage()
{
	...
	cerr << "<-signed>           If the target is inside the source surface, the value is negative otherwise positive" << endl;
	exit(1);
}

int main(int argc, char **argv)
{
	int i, n, id, locatorType, rms, scalar, issigned, ok;
	...
	vtkSmartPointer<vtkSelectEnclosedPoints> selectEnclosedPoints = NULL;
	...
	while (argc > 1) {
		...
		if ((ok == false) && (strcmp(argv[1], "-signed") == 0)){
			argc--;
			argv++;
			issigned = true;
			ok = true;
	...
	if(issigned){
		...
		selectEnclosedPoints->SetCheckSurface(1);
		selectEnclosedPoints->SetTolerance(0.00001);
#if VTK_MAJOR_VERSION <= 5
		selectEnclosedPoints->SetInput(target);
#else
		selectEnclosedPoints->SetInputData(target);
#endif
#if VTK_MAJOR_VERSION <= 5
		selectEnclosedPoints->SetSurface(source);
#else
		selectEnclosedPoints->SetSurfaceData(source);
#endif
