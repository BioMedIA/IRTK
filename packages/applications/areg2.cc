...
int main(int argc, char **argv)
{
	...
	while (argc > 1) {
		...
		if ((ok == false) && (strcmp(argv[1], "-init_rigid") == 0)) {
			argc--;
			argv++;
			init_rigid = true;
			ok = true;
		...
		if ((ok == false)
		    && ((strcmp(argv[1], "-parameter") == 0)
		        || (strcmp(argv[1], "-parin") == 0))) {
			argc--;
			argv++;
			ok = true;
			parin_name = argv[1];
	...
	if ((target_x1 != 0) || (target_x2 != target.GetX()) || (target_y1 != 0)
	    || (target_y2 != target.GetY()) || (target_z1 != 0)
	    || (target_z2 != target.GetZ())) {
		target = target.GetRegion(target_x1, target_y1, target_z1, target_x2,
		    target_y2, target_z2);
	}

	// If there is an region of interest for the source image, use it
	if ((source_x1 != 0) || (source_x2 != source.GetX()) || (source_y1 != 0)
	    || (source_y2 != source.GetY()) || (source_z1 != 0)
	    || (source_z2 != source.GetZ())) {
		source = source.GetRegion(source_x1, source_y1, source_z1, source_x2,
		    source_y2, source_z2);
	}
	...
	if (dofin_name != NULL) {
		if (init_rigid == true) {
			cerr << "Incompatible options -init_rigid and -dofin" << endl;
			exit(1);
		}
	...
	if (centerImages == true) {
		if (init_rigid == true) {
			cerr << "Incompatible options -init_rigid and -center" << endl;
			exit(1);
		}
		...
		irtkAffineTransformation *affineTransf =
		    dynamic_cast<irtkAffineTransformation*>(transformation);
	...
	if (init_rigid == true) {
		cout << "Performing initial rigid transformation... ";
		irtkRigidTransformation *rigid_transformation = new irtkRigidTransformation;
		irtkImageRigidRegistration2 *rigid_registration = new irtkImageRigidRegistration2;
		rigid_registration->SetInput(&target, &source);
		rigid_registration->SetOutput(rigid_transformation);

		// Make an initial Guess for the parameters.
		rigid_registration->GuessParameter();

		// Overrride with any the user has set.
		if (parin_name != NULL) {
			rigid_registration->irtkImageRegistration2::Read(parin_name);
		}

		// Set padding
		if (padding != MIN_GREY) {
			rigid_registration->SetTargetPadding(padding);
		}

		// Run registration filter
		rigid_registration->Run();
		transformation->PutMatrix(rigid_transformation->GetMatrix());
		delete rigid_registration;
		cout << "done" << endl;
	}
