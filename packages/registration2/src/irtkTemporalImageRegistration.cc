...
void irtkTemporalImageRegistration::Initialize(int level)
{
  ...
  if (_SourceBlurring[level] > 0) {
	...
	if (blurrZ) {
	  cout<<"xy ";
	  irtkGaussianBlurring2D<irtkGreyPixel> blurring(_SourceBlurring[level]);
...
//  cout << "Target images (reference)" << endl;
//  for (int n=0; n<_N_target; n++)
//    _target[n]->Print();
	  ...
	  for (j = 0; j < _target[n]->GetY(); j++) {
		for (i = 0; i < _target[n]->GetX(); i++) {
		  if ((_target[n]->Get(i, j, k) >= 0) && (_transformedSource[n](i, j, k) >= 0)) {
			ssd += (_target[n]->Get(i, j, k) - _transformedSource[n](i, j, k)) * (_target[n]->Get(i, j, k) - _transformedSource[n](i, j, k));
			nV++;
		  }
		}
	  }
  ...
  for (n = 0; n < _N_target; n++) {
	...
	for (k = 0; k < _target[n]->GetZ(); k++) {
	  for (j = 0; j < _target[n]->GetY(); j++) {
		for (i = 0; i < _target[n]->GetX(); i++) {
		  if ((_target[n]->Get(i, j, k) >= 0) && (_transformedSource[n](i, j, k) >= 0)) {
			ssd = 2 * (_target[n]->Get(i, j, k) - _transformedSource[n](i, j, k)) / _maxDiff;
