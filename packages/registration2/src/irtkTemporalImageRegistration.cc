...
void irtkTemporalImageRegistration::Initialize()
{
	...
	for (int n=0; n<_N_source; n++)
		if ((_source[n]->GetT() > 1) || (_target->GetT() > 1)) {
...
void irtkTemporalImageRegistration::Initialize(int level)
{
	...
	_target->GetPixelSize(&dxT, &dyT, &dzT);
	tempS = fabs(_TargetResolution[level][0]-dxT) 
	...
	if (level > 0 || tempS > 0.000001) {
		...
		irtkResamplingWithPadding<irtkGreyPixel> resample(_TargetResolution[level][0],
			...
			_TargetResolution[level][2], _TargetPadding);
	...
	tmp_tsource = new irtkGreyImage *[_N_source];
	for (n = 0; n < _N_source; n++) {
		// Copy target to temp space
		tmp_tsource[n] = new irtkGreyImage(*_source[n]);
		// Swap target with temp space copies
		swap(tmp_tsource[n], _source[n]);
	}
	...
	cout << "Blurring (and resampling) sources ... "; cout.flush();
	for (n=0; n<_N_source; n++) {

		_source[n]->GetPixelSize(&dxS, &dyS, &dzS);
		tempT = fabs(_SourceResolution[level][0]-dxT) 
			+ fabs(_SourceResolution[level][1]-dyT) 
			+ fabs(_SourceResolution[level][2]-dzT);
		...
		if (_SourceBlurring[level] > 0) {
			if (blurrZ) {
				irtkGaussianBlurringWithPadding2D<irtkGreyPixel> blurring(_SourceBlurring[level], _SourcePadding);
	...
	case NMI:
		...
		//      target_nbins = irtkCalculateNumberOfBins(_source, _NumberOfBins, _target_min, _target_max);
	...
	_sourceGradient = new irtkGenericImage<double> [_N_source];
	for (n = 0; n < _N_source; n++) {
	...
	_transformedSource = new irtkGenericImage<double> [_N_source];
	for (n = 0; n < _N_source; n++) {
		attr_t = _target->GetImageAttributes();
		_transformedSource[n].Initialize(attr_t);
	}

	// Set up gradient of source image (transformed)
	_transformedSourceGradient = new irtkGenericImage<double> [_N_source];
	for (n = 0; n < _N_source; n++) {
		attr_t = _target->GetImageAttributes();
		attr_t._t = 3;
	...
	_similarityGradient = new irtkGenericImage<double> [_N_source];
	for (n = 0; n < _N_source; n++) {
		attr_t = _target->GetImageAttributes();
		attr_t._t = 3;
	...
	_interpolatorGradient = new irtkInterpolateImageFunction* [_N_source];
	_interpolator = new irtkInterpolateImageFunction* [_N_source];
	for (n = 0; n < _N_source; n++) {
		...
		_interpolator[n]->Inside(_source_x1, _source_y1, _source_z1,
		...
		// Calculate the source image domain in which we can interpolate
		_interpolatorGradient[n]->Inside(_source_x1, _source_y1, _source_z1,
			_source_x2, _source_y2, _source_z2);
	}

	//Print some debugging information
	for (int n=0; n<_N_source; n++){
		cout << "Source images"<<n<<" (reference)" << endl;
		_source[n]->Print();
	}
	cout << "Sources Range is from " << _source_min << " to " << _source_max << endl;


	cout << "Target image (transform)" << endl;
	_target->Print();
	cout << "Target range is from " << _target_min << " to " << _target_max << endl;
...
void irtkTemporalImageRegistration::Finalize(int level)
{
	...
	for (i = 0; i < _N_source; i++) {
		swap(tmp_tsource[i], _source[i]);
	}
	swap(tmp_target, _target);

	for (i = 0; i < _N_source; i++) {
		delete tmp_tsource[i];
		tmp_tsource[i] = NULL;
		delete _interpolator[i];
		_interpolator[i] = NULL;
		delete _interpolatorGradient[i];
		_interpolatorGradient[i] = NULL;
	}
	delete [] tmp_tsource;
	tmp_tsource = NULL;
	delete tmp_target;
...
void irtkTemporalImageRegistration::Run()
{
	...
	for (_CurrentLevel = _NumberOfLevels-1; _CurrentLevel >= 0; _CurrentLevel--) {
		...
		if (_DebugFlag == true)
			for (int n = 0; n < _N_source; n++) {
				sprintf(buffer, "source_%d_t%d.nii.gz", _CurrentLevel, n);
...
double irtkTemporalImageRegistration::EvaluateSSD()
{
	...
	for (n = 0; n < _N_source; n++) {
		...
		for (i = 0; i < _source[n]->GetNumberOfVoxels(); i++) {
...
double irtkTemporalImageRegistration::EvaluateNMI()
{
	...
	//
	//  // Pointer to voxels in images
	//  short  *ptr2target = _source->GetPointerToVoxels();
	...
	//
	//  // Compute metric
	//  for (i = 0; i < _source->GetNumberOfVoxels(); i++) {
	//    if ((*ptr2target >= 0) && (*ptr2source >= 0)) {
