...
// Default filenames
char *input_name = NULL, *vtk_name = NULL, *output_name = NULL, *axis_name = NULL;

void usage()
{
	cerr << "Usage: cardiacdof2surface [dof] [input_surface] [output_surface] \n"<<             
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < model->GetNumberOfPoints(); i++) {
		...
		if(mode != Normal){
			//TODO need to define normal here
	...
	model->GetPointData()->SetVectors(vectors);
	model->GetPointData()->SetScalars(scalarvectors);
	if(strain){
		model->GetPointData()->SetTensors(straintensor);
	}
	model->Update();
