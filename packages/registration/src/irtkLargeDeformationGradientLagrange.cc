...
LargeDefGradLagrange::LargeDefGradLagrange(void){
  ...
  for (i=0;i<10;i++) strcpy(SourceFiles[i],"Null");
  for (i=0;i<10;i++) strcpy(TargetFiles[i],"Null");
  for (i=0;i<10;i++) weightChannel[i]=1.;
  NbChannels=0;
  MeasureTypicAmp=0;
}
...
void LargeDefGradLagrange::AllocateAllVariables(void){
  ...
  //... gradient of J0
  //    -->  GradJ0.G(0,x,y,z)= gradient of J0 in direction ex at (x,y,z)
  //    -->  GradJ0.G(1,x,y,z)= gradient of J0 in direction ey at (x,y,z)
  //    -->  GradJ0.G(2,x,y,z)= gradient of J0 in direction ez at (x,y,z)
  this->GradJ0.CreateVoidField(this->NX,this->NY,this->NZ);
...
void LargeDefGradLagrange::ComputeEnergyGradient(int timeSubdiv,int IdChannel){
  ...
  if (SplitKernels==0){  //CASE 1: NON-SPLITTED KERNEL
    ...
    for (i=0;i<3;i++){
      ...
      for (z = 0; z < this->NZ; z++) for (y = 0; y < this->NY; y++) for (x = 0; x < this->NX; x++){
        temp=(this->J0.G(x,y,z) - this->J1.G(x,y,z)) * this->DetJacobians.G(x,y,z) * this->GradJ0.G(i,x,y,z);
...
void LargeDefGradLagrange::LoadSplittedVelocityFields(char Prefix[256]){
  ...
  char FileNameY[256];
  char FileNameZ[256];
  char VelocityField_X1[256];
  char VelocityField_Y1[256];
  char VelocityField_Z1[256];
  char VelocityField_X2[256];
  char VelocityField_Y2[256];
  char VelocityField_Z2[256];
  char VelocityField_X3[256];
  char VelocityField_Y3[256];
  char VelocityField_Z3[256];
  char VelocityField_X4[256];
  char VelocityField_Y4[256];
  char VelocityField_Z4[256];
  
  //intialisation
  strcpy(VelocityField_X1,"_VelocityField_X1.nii");
  strcpy(VelocityField_Y1,"_VelocityField_Y1.nii");
  strcpy(VelocityField_Z1,"_VelocityField_Z1.nii");
  strcpy(VelocityField_X2,"_VelocityField_X2.nii");
  strcpy(VelocityField_Y2,"_VelocityField_Y2.nii");
  strcpy(VelocityField_Z2,"_VelocityField_Z2.nii");
  strcpy(VelocityField_X3,"_VelocityField_X3.nii");
  strcpy(VelocityField_Y3,"_VelocityField_Y3.nii");
  strcpy(VelocityField_Z3,"_VelocityField_Z3.nii");
  strcpy(VelocityField_X4,"_VelocityField_X4.nii");
  strcpy(VelocityField_Y4,"_VelocityField_Y4.nii");
  strcpy(VelocityField_Z4,"_VelocityField_Z4.nii");
...
void LargeDefGradLagrange::SaveDeformations(char Prefix[256]){
  ...
  for (TimeLoc=0;TimeLoc<this->NbTimeSubdiv;TimeLoc++){
    ForwardProjectionOf3Dimage(&source_image,&this->ForwardMapping,&this->J0,TimeLoc);
...
void LargeDefGradLagrange::SaveFinalForwardMap(char Prefix[256]){
  ...
  TempField.CreateVoidField(this->NX, this->NY, this->NZ,3);
  
  strcpy(FwdMapAtT1,"_FwdMapAtT1.nii");
  
  //save the forward mapping
  for (z = 0; z < this->NZ; z++) for (y = 0; y < this->NY; y++) for (x = 0; x < this->NX; x++){
    TempField.P(this->ForwardMapping.G(0,x,y,z,this->NbTimeSubdiv-1),x, y, z,0);
    TempField.P(this->ForwardMapping.G(1,x,y,z,this->NbTimeSubdiv-1),x, y, z,1);
    TempField.P(this->ForwardMapping.G(2,x,y,z,this->NbTimeSubdiv-1),x, y, z,2);
  }
  
  strcpy(FileName,Prefix);
  strcat(FileName,FwdMapAtT1);
  TempField.Write(FileName,SourceFiles[0]);
}

///save the final backward mapping from [source] into [target] at t=1
void LargeDefGradLagrange::SaveFinalBackwardMap(char Prefix[256]){
  int x, y, z;
  ScalarField TempField;
  char FileName[256];
  char BkwMapAtT0[256];
  ...
  strcpy(BkwMapAtT0,"_BkwMapAtT0.nii");
  
  //save the forward mapping
  for (z = 0; z < this->NZ; z++) for (y = 0; y < this->NY; y++) for (x = 0; x < this->NX; x++){
    TempField.P(this->BackwardMapping.G(0,x,y,z,0),x, y, z,0);
    TempField.P(this->BackwardMapping.G(1,x,y,z,0),x, y, z,1);
    TempField.P(this->BackwardMapping.G(2,x,y,z,0),x, y, z,2);
  }
  
  strcpy(FileName,Prefix);
  strcat(FileName,BkwMapAtT0);
  TempField.Write(FileName,SourceFiles[0]);
}

///save the total length of the flow of deformation from each voxel of the image
void LargeDefGradLagrange::SaveGlobalFlowLength(char [256]){
  char VeloLength[256];
...
void LargeDefGradLagrange::SaveSplittedFlowLength(char [256]){
...
float LargeDefGradLagrange::Run_MeasureTypicalAmplitude(void){
  ...
  this->SplitKernels=0;
  this->NbChannels=1;
  for (i=1;i<10;i++) strcpy(this->SourceFiles[i],"Null");
  for (i=1;i<10;i++) strcpy(this->TargetFiles[i],"Null");
  ...
  ForwardProjectionOf3Dimage(&this->ImTemplate[0],&this->ForwardMapping,&this->J0,0);
  
  //2.3.3) compute the temporary image transformed using the backward mapping from time 1 -> J1
  BackwardProjectionOf3Dimage(&this->ImTarget[0],&this->BackwardMapping,&this->J1,0);
  
  //2.3.4) compute gradient of J0
  Cpt_Grad_ScalarField(&this->J0,&this->GradJ0);
  ...
  while (IterationStopper==0){
    ...
    for (TimeSubdiv=0;TimeSubdiv<this->NbTimeSubdiv;TimeSubdiv++){//LOOP ON THE TIME SUBDIVISIONS
      ...
      for (IdChannel=0;IdChannel<this->NbChannels;IdChannel++){//LOOP ON THE CHANNELS
        //2.3.2) compute the temporary image transformed using the forward mapping from time 0 -> J0
        ForwardProjectionOf3Dimage(&this->ImTemplate[IdChannel],&this->ForwardMapping,&this->J0,TimeSubdiv);
        
        //2.3.3) compute the temporary image transformed using the backward mapping from time 1 -> J1
        BackwardProjectionOf3Dimage(&this->ImTarget[IdChannel],&this->BackwardMapping,&this->J1,TimeSubdiv);
